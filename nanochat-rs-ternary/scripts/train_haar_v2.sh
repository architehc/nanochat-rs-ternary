#!/bin/bash
# Chunked training for nano-500m-wave-haar on larger dataset
# Works around Candle's CUDA memory leak by restarting every CHUNK_SIZE steps
#
# Usage: ./scripts/train_haar_v2.sh [target_steps] [device] [data_path]
...
CHECKPOINT_DIR="checkpoints/nano-500m-haar-v2"
BATCH_SIZE=2
SEQ_LEN=512
LOG_INTERVAL=50
...
echo "═══════════════════════════════════════════════════════════"
echo "  nano-500m-wave-haar v2 Chunked Training (large dataset)"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "Configuration:"
echo "  Config:           $CONFIG"
...
if [ ! -f "$DATA_PATH" ]; then
    ...
    echo "Run: cargo run --release -p nanochat-train -- prepare-data --text data/rust_large.txt --vocab-size 4096 --output data/rust_v2_prepared"
    exit 1
fi
...
echo "Chunks needed: $CHUNKS_NEEDED (~$(( CHUNKS_NEEDED * 7 )) minutes)"
echo ""
