#!/bin/bash
# Chunked training for nano-275m-wave-haar on GPU 0
# ~277M params, dim=1024, 20 layers, Haar wavefield attention
# Designed for A/B testing against train_275m_baseline.sh on GPU 1
#
# Usage: ./scripts/train_275m_wave.sh [target_steps] [device] [data_path]
...
CHUNK_SIZE=5000
CHECKPOINT_DIR="checkpoints/nano-275m-haar"
BATCH_SIZE=2
SEQ_LEN=256
LOG_INTERVAL=50
CONFIG="nano-275m-wave-haar"

export LD_LIBRARY_PATH=/usr/lib/wsl/lib:$LD_LIBRARY_PATH

echo "═══════════════════════════════════════════════════════════"
echo "  nano-275m-wave-haar Chunked Training (GPU 0)"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "Configuration:"
echo "  Config:           $CONFIG"
