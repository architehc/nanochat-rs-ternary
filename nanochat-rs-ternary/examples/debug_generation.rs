...
fn tensor_stats(tensor: &Tensor, name: &str) -> Result<()> {
    ...
    println!(
        "  {} stats: mean={:.4}, std={:.4}, min={:.4}, max={:.4}",
...
fn main() -> Result<()> {
    ...
    println!(
        "  Token 0 embedding (first 5): {:?}",
        token_0.narrow(0, 0, 5)?.to_vec1::<f32>()?
    );
    println!(
        "  Token 100 embedding (first 5): {:?}",
        token_100.narrow(0, 0, 5)?.to_vec1::<f32>()?
    );
    println!(
        "  Token 1391 embedding (first 5): {:?}",
    ...
    let cosine_sim = emb_0_vec
        .iter()
        .zip(emb_100_vec.iter())
        .map(|(a, b)| a * b)
        .sum::<f32>()
    ...
    if lm_weights_shape[0] == config.vocab_size {
        ...
        println!(
            "  Output weight for token 1391 (first 5): {:?}",
    ...
    println!(
        "  Probability of max token (after softmax): {:.6}",
        prob_max
    );
