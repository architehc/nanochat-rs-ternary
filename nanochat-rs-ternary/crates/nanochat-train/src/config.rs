...
impl TrainConfig {
    ...
    /// nano-275m Engram v10: 15K steps, lr=0.012, EARLY decay (50% instead of 80%).
    /// Hypothesis: v1 succeeded at lr=0.012 because decay started at step 8K (80% of 10K),
    /// limiting full-LR exposure to ~6.5K steps (8K minus 1.5K warmup). With 15K steps and
    /// decay_start_frac=0.5, decay starts at step 7500 → only 6K steps at full LR (safe)
    /// → 7500 steps of decay (vs v1's 2000) → should converge deeper.
    pub fn nano_275m_engram_v10() -> Self {
        let mut cfg = Self::nano_275m_engram_only();
        cfg.total_steps = 15_000;
        cfg.lr = 0.012;
        cfg.decay_start_frac = 0.5; // Early decay: at step 7500
        cfg
    }
    ...
    /// nano-275m Engram v12: lr=0.010, 20K steps, decay at 40% (step 8K).
    /// Middle LR (between stable 0.008 and unstable 0.012) with early decay.
    /// V9 at lr=0.010 with late decay got loss 3.32 — early decay should improve.
    pub fn nano_275m_engram_v12() -> Self {
        let mut cfg = Self::nano_275m_engram_only();
        cfg.total_steps = 20_000;
        cfg.lr = 0.010;
        cfg.decay_start_frac = 0.40; // Decay at step 8000
        cfg
    }
    ...
    /// nano-275m Engram v16: lr=0.012, 12K steps, decay at 67% (step 8K).
    /// Same full-LR budget as v1 (8K steps) but 4K decay steps (vs v1's 2K).
    /// Uses SMALL dataset. Hypothesis: more decay steps after aggressive exploration.
    pub fn nano_275m_engram_v16() -> Self {
        let mut cfg = Self::nano_275m_engram_only();
        cfg.total_steps = 12_000;
        cfg.lr = 0.012;
        cfg.decay_start_frac = 0.67; // Decay at step ~8K
        cfg
    }
