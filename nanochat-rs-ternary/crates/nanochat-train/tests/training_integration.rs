...
use nanochat_train::{
    ...
    train::{StepAttempt, Trainer, TrainerBuilder},
};
...
/// A resumed checkpoint must produce the *same loss* on the same batch.
///
/// The structural checks above (files exist, step matches) pass even when every
/// weight comes back wrong, which is exactly what happened in practice: a real
/// 258M run saved at loss 3.29 resumed at loss 7.56. This pins the property that
/// actually matters — reload is loss-preserving — so a silent weight-restore
/// regression fails here instead of days into a training run.
#[test]
fn test_checkpoint_resume_preserves_loss() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let device = Device::Cpu;
    let config = tiny_config();
    ...
    // Train a few steps so the weights are not still at their init values —
    // an all-zeros or all-init model would round-trip trivially.
    for _ in 0..3 {
    ...
    let loss_before = trainer
        .model
        .forward_loss(&input_ids, &target_ids)?
        .to_scalar::<f32>()?;

    let temp_dir = tempfile::tempdir()?;
    let ckpt_path = temp_dir.path().join("step_3");
    checkpoint::save_checkpoint(
        &trainer.varmap,
        &config,
        trainer.global_step,
    ...
    let loaded = Trainer::from_checkpoint(ckpt_path.to_str().unwrap(), device)?;
    let loss_after = loaded
        .model
        .forward_loss(&input_ids, &target_ids)?
        .to_scalar::<f32>()?;
    ...
    assert!(
        ...
        "checkpoint resume changed the loss: {loss_before} before, {loss_after} after"
    );
    Ok(())
}
...
fn test_grad_accumulation_steps() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Should not step on first micro-step"
    );

    let _ = trainer.train_step(&input_ids, &target_ids)?;
    assert_eq!(
        trainer.global_step, 0,
        "Should not step on second micro-step"
    );
