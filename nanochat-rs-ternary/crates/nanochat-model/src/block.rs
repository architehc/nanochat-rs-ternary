        ...
        Self::from_parts(
            ...
            match &self.attention {
                AttentionLayer::Standard(a) => AttentionLayer::Standard(Attention {
                    ...
                    w_gate: a.w_gate.as_ref().map(|w| w.clone_to_node(node)),
                    n_heads: a.n_heads,
                ...
                AttentionLayer::DeltaNet(a) => AttentionLayer::DeltaNet(DeltaNetAttention {
                    ...
                    w_alpha: a.w_alpha.as_ref().map(|w| w.clone_to_node(node)),
                    a_log: a.a_log.clone(),
                    dt_bias: a.dt_bias.clone(),
                    w_gate: a.w_gate.as_ref().map(|w| w.clone_to_node(node)),
                    out_norm_weight: a.out_norm_weight.clone(),
                    n_heads: a.n_heads,
                    head_dim: a.head_dim,
                }),
